...
public class Typer extends Phase<Tree.TopLevel, Tree.TopLevel> implements Visitor {
    ...
    private Stack<Tree.Stmt> breaks = new Stack<>();
    ...
    public void visitUnary(Tree.Unary expr) {
        ...
        if (expr.op == Tree.UnaryOp.NEG) {
            if (expr.operand.type.equal(BaseType.ERROR)
                ...
                issue(new IncompatUnOpError(expr.getLocation(), "-",
    ...
    public void visitIndexSel(Tree.IndexSel indexed) {
        ...
        if (!indexed.array.type.isArrayType()) {
            issue(new NotArrayError(indexed.array.getLocation()));
        ...
        if (!indexed.index.type.equal(BaseType.INT)) {
            issue(new SubNotIntError(indexed.getLocation()));
        }
    }
    ...
    private void checkCallExpr(Tree.Call callExpr, Symbol f) {
        ...
        if (f == null) {
            issue(new FieldNotFoundError(callExpr.getLocation(),
                    callExpr.method.name, receiverType.toString()));
            callExpr.type = BaseType.ERROR;
        } else if (!f.isFunction()) {
            issue(new NotClassMethodError(callExpr.getLocation(),
                    callExpr.method.name, receiverType.toString()));
            callExpr.type = BaseType.ERROR;
        } else {
            ...
            if (argList.size() != argCount) {
                issue(new BadArgCountError(callExpr.getLocation(),
                ...
                for (int i = 1; iter1.hasNext(); i++) {
                    ...
                    if (!t2.equal(BaseType.ERROR) && !t2.compatible(t1)) {
                        issue(new BadArgTypeError(e.getLocation(), i,
        ...
        if (callExpr.method.equals("length")) {
            if (callExpr.receiver.get().type.isArrayType()) {
                if (callExpr.args.size() > 0) {
                    issue(new BadLengthArgError(callExpr.getLocation(),
                            callExpr.args.size()));
                }
                ...
                issue(new BadLengthError(callExpr.getLocation()));
                callExpr.type = BaseType.ERROR;
                return;
            }
        }
    ...
    public void visitNewArray(Tree.NewArray newArrayExpr) {
        ...
        if (newArrayExpr.elemType.type.equal(BaseType.ERROR)) {
            ...
            issue(new BadArrElementError(newArrayExpr.elemType
                    .getLocation()));
            newArrayExpr.type = BaseType.ERROR;
        } else {
        ...
        if (!newArrayExpr.length.type.equal(BaseType.ERROR)
            ...
            issue(new BadNewArrayLength(newArrayExpr.length.getLocation()));
        }
    }
    ...
    public void visitNewClass(Tree.NewClass newClass) {
        ...
        newClass.symbol = c;
        if (c == null) {
            issue(new ClassNotFoundError(newClass.getLocation(),
                    newClass.clazz.name));
    ...
    public void visitThis(Tree.This thisExpr) {
        if (currentFunction.isStatik()) {
            issue(new ThisInStaticFuncError(thisExpr.getLocation()));
    ...
    public void visitClassTest(Tree.ClassTest instanceofExpr) {
        ...
        if (!instanceofExpr.obj.type.isClassType()) {
            issue(new NotClassError(instanceofExpr.obj.type
        ...
        if (c == null) {
            issue(new ClassNotFoundError(instanceofExpr.getLocation(),
    ...
    public void visitClassCast(Tree.ClassCast cast) {
        ...
        if (!cast.obj.type.isClassType()) {
            issue(new NotClassError(cast.obj.type.toString(),
                    cast.getLocation()));
        }
        Class c = table.lookupClass(cast.to.name);
        cast.symbol = c;
        if (c == null) {
            issue(new ClassNotFoundError(cast.getLocation(),
                    cast.to.name));
    ...
    public void visitVarSel(Tree.VarSel varSel) {
        if (varSel.receiver.isEmpty()) {
            Symbol v = table.lookupBeforeLocation(varSel.variable.name, varSel
                    .getLocation());
            if (v == null) {
                issue(new UndeclVarError(varSel.getLocation(), varSel.variable.name));
                ...
                if (var.isLocalVar()) {
                    ...
                    if (currentFunction.isStatik()) {
                        issue(new RefNonStaticError(varSel.getLocation(),
                ...
                if (v.isClass()) {
                    if (varSel.usedForRef) {
                        ...
                        issue(new UndeclVarError(varSel.getLocation(),
            ...
            if (!varSel.receiver.get().type.equal(BaseType.ERROR)) {
                if (varSel.receiver.get().isClass || !varSel.receiver.get().type.isClassType()) {
                    issue(new NotClassFieldError(varSel.getLocation(),
                    ...
                    if (v == null) {
                        issue(new FieldNotFoundError(varSel.getLocation(),
                                varSel.variable.name, varSel.receiver.get().type.toString()));
                        varSel.type = BaseType.ERROR;
                    } else if (v.isVariable()) {
                        ...
                        if (!thisType.compatible(varSel.receiver.get().type)) {
                            issue(new FieldNotAccessError(varSel
    ...
    public void visitAssign(Tree.Assign assign) {
        ...
        if (!assign.lhs.type.equal(BaseType.ERROR)
            ...
            issue(new IncompatBinOpError(assign.getLocation(),
    ...
    public void visitBreak(Tree.Break breakStmt) {
        if (breaks.empty()) {
            issue(new BreakOutOfLoopError(breakStmt.getLocation()));
        }
    }
    ...
    public void visitPrint(Tree.Print printStmt) {
        ...
        for (Tree.Expr e : printStmt.exprs) {
            ...
            if (!e.type.equal(BaseType.ERROR) && !e.type.equal(BaseType.BOOL)
                ...
                issue(new BadPrintArgError(e.getLocation(), Integer
    ...
    public void visitReturn(Tree.Return returnStmt) {
        ...
        if (returnType.equal(BaseType.VOID)) {
            if (returnStmt.expr.isPresent()) {
                issue(new BadReturnTypeError(returnStmt.getLocation(),
    ...
    private Type checkBinaryOp(Tree.Expr left, Tree.Expr right, Tree.BinaryOp op, Pos pos) {
        ...
        if (!compatible) {
            issue(new IncompatBinOpError(pos, left.type.toString(),
    ...
    private void checkTestExpr(Tree.Expr expr) {
        ...
        if (!expr.type.equal(BaseType.ERROR) && !expr.type.equal(BaseType.BOOL)) {
            issue(new BadTestExpr(expr.getLocation()));
        }
    }
